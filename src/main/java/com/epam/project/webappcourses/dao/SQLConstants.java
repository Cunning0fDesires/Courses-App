...
public class SQLConstants {
    ...
    public static final String GET_ALL_TEACHERS_WITH_COURSES = "SELECT u.user_id, u.name || ' ' || u.surname as teacher, c.name AS course FROM public.user u FULL JOIN public.teacher_course tc ON  u.user_id = tc.teacher_id FULL JOIN public.course c ON  tc.course_id = c.course_id WHERE u.role_id = 1;";
    ...
    public static final String CHANGE_TEACHER_FOR_COURSE = "UPDATE public.teacher_course SET teacher_id = ? WHERE course_id = ?;";
    public static final String SAVE_PREVIOUS_TEACHER = "INSERT INTO public.previous_teachers (course_id, teacher_id) VALUES (?, ?);";
    public static final String GET_TEACHER_ID_BY_COURSE_ID = "SELECT teacher_id FROM public.teacher_course WHERE course_id = ?;";
    public static final String GET_PREVIOUS_TEACHERS = "SELECT u.name || ' ' || u.surname AS teacher FROM public.previous_teachers pt RIGHT JOIN public.user u on pt.teacher_id = u.user_id WHERE pt.course_id = ?;";
    public static final String GET_COURSES_SORTED_FOR_ADMIN = "SELECT c.course_id, c.name, t.topic_name, c.start_date, c.end_date FROM public.course c RIGHT JOIN public.topic t ON  c.topic_id = t.topic_id WHERE c.start_date > ? AND c.end_date < ?;";
